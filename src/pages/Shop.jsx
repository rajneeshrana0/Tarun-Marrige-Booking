...
const ShopPage = () => {
  ...
  const [quickBuyProductId, setQuickBuyProductId] = useState(null); // State to track the Book Now

  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const [productsResponse, collectionProductsResponse] = await Promise.all([
          fetch("https://tarun-marrige-booking.onrender.com/api/products/products"),
          fetch("https://tarun-marrige-booking.onrender.com/api/collection-products/products")
        ]);
  ...
  const handleQuickBuyClick = (productId) => {
    setQuickBuyProductId(productId === quickBuyProductId ? null : productId); // Toggle Book Now
  };
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-10">
        {products.map((product) => (
          <div key={product._id} className="group relative">
            ...
            {/* Conditionally render the Book Now details */}
