...
// Create a new product (Video Booking Service) under a specific category
exports.createProduct = async (req, res) => {
  try {
    const { name, size, price, description, eventType, duration, additionalServices } = req.body;
    const { categoryId } = req.params;
    ...
    // Create the product (Video Booking Service)
    const product = new Product({
      name,
      size,
      price,
      ...
      additionalServices, // New field for additional services
      images: imageUrls,
      category: categoryId,
    });
...
// Get all products (Video Booking Services) for a specific category
exports.getProductsByCategory = async (req, res) => {
  try {
    const { categoryId } = req.params;
...
// Get a specific product (Video Booking Service) by ID
exports.getProductById = async (req, res) => {
  try {
    const { productId } = req.params;
...
// Update a specific product (Video Booking Service)
exports.updateProduct = async (req, res) => {
  try {
    const { productId, categoryId } = req.params;
    const { name, size, price, description, eventType, duration, additionalServices } = req.body;
    ...
    if (price) product.price = price;
    if (description) product.description = description;
    if (eventType) product.eventType = eventType; // Update event type
    if (duration) product.duration = duration; // Update duration
    if (additionalServices) product.additionalServices = additionalServices; // Update additional services
...
// Delete a product (Video Booking Service)
exports.deleteProduct = async (req, res) => {
  try {
    const { productId } = req.params;
...
// Get all products (Video Booking Services) without category ID
exports.getAllProducts = async (req, res) => {
  try {
    // Fetch all products and populate the related category details
    const products = await Product.find()
      .populate('category'); // Assuming 'category' field stores category ID
