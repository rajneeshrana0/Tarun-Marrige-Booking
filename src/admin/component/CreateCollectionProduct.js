...
//     try {
//       const response = await axios.get('https://tarun-marrige-booking.onrender.com/api/collection-products/products');
...
//         ? `https://tarun-marrige-booking.onrender.com/api/collection-products/${selectedCollection}/products/${productId}`
//         : `https://tarun-marrige-booking.onrender.com/api/collection-products/${selectedCollection}/products`;
...
//       try {
//         await axios.delete(`https://tarun-marrige-booking.onrender.com/api/collection-products/${collectionId}/products/${productId}`);
...
const CreateEditCollectionProduct = () => {
  ...
  const [size, setSize] = useState('small');
  const [description, setDescription] = useState('');
  const [albumType, setAlbumType] = useState('Wedding');
  ...
  const handleSubmit = async (e) => {
    ...
    formData.append('size', size);
    formData.append('description', description);
    formData.append('albumType', albumType); // Add albumType field
    formData.append('numberOfPages', numberOfPages); // Add numberOfPages field
    formData.append('customizationNotes', customizationNotes); // Add customizationNotes field
  ...
  const resetForm = () => {
    ...
    setSize('small');
    setDescription('');
    setAlbumType('Wedding'); // Reset albumType
    setNumberOfPages(''); // Reset numberOfPages
    setCustomizationNotes(''); // Reset customizationNotes
    setImages([]);
    setUploadProgress(0);
    setIsEditMode(false);
    setProductId(null);
  ...
  const handleEdit = (product) => {
    ...
    setSize(product.size);
    setDescription(product.description);
    setAlbumType(product.albumType); // Set albumType for editing
    setNumberOfPages(product.numberOfPages); // Set numberOfPages for editing
    setCustomizationNotes(product.customizationNotes); // Set customizationNotes for editing
  };
    ...
    <div className="min-h-screen bg-gray-100 flex flex-col items-center justify-center py-10">
      ...
      <div className="bg-white p-8 rounded-lg shadow-lg max-w-md w-full">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {/* New fields for albumType, numberOfPages, and customizationNotes */}
          <div className="space-y-2">
            <label className="block text-gray-700 text-sm font-semibold">Album Type</label>
            <select
              value={albumType}
          ...
          <div className="space-y-2">
            <label className="block text-gray-700 text-sm font-semibold">Number of Pages</label>
            <input
              type="number"
              value={numberOfPages}
          ...
          <div className="space-y-2">
            <label className="block text-gray-700 text-sm font-semibold">Customization Notes</label>
            <textarea
              value={customizationNotes}
              onChange={(e) => setCustomizationNotes(e.target.value)}
              className="w-full p-3 border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-blue-500 transition"
              placeholder="Enter any customization instructions"
            />
          </div>
        ...
        {products.length === 0 ? (
          ...
          <ul>
            {products.map((product) => (
              ...
              >
                <span className="text-gray-700">
                  {product.name} - ₹{product.price} - {product.size} - {product.albumType} - {product.numberOfPages} pages
                </span>
                <div>
                  <button
                    onClick={() => handleEdit(product)}
