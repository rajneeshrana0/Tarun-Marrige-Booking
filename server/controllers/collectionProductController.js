...
// Create a new product under a specific collection (Album Printing Service)
exports.createCollectionProduct = async (req, res) => {
  try {
    const { name, size, price, description, albumType, numberOfPages, customizationNotes } = req.body;
    const images = await Promise.all(req.files.map(file => cloudinary.uploader.upload(file.path)));
    const imageUrls = images.map(image => image.secure_url);
...
// Get all products for a specific collection (Album Printing Services)
exports.getCollectionProducts = async (req, res) => {
  try {
    const products = await CollectionProduct.find({ collection: req.params.collectionId });
    if (!products) {
...
// Get all collection products without collection ID (Album Printing Services)
exports.getAllCollectionProducts = async (req, res) => {
  try {
    // Fetch all products and populate the collection details
    const products = await CollectionProduct.find()
...
// Get a specific product by ID (Album Printing Service)
exports.getCollectionProductById = async (req, res) => {
  try {
    const product = await CollectionProduct.findById(req.params.productId);
    if (!product) {
...
// Update a product in a specific collection (Album Printing Service)
exports.updateCollectionProduct = async (req, res) => {
  try {
    const { name, size, price, description, albumType, numberOfPages, customizationNotes } = req.body;
    const product = await CollectionProduct.findById(req.params.productId);
    ...
    product.price = price || product.price;
    product.description = description || product.description;
    product.albumType = albumType || product.albumType; // Update album type
    product.numberOfPages = numberOfPages || product.numberOfPages; // Update number of pages
    product.customizationNotes = customizationNotes || product.customizationNotes; // Update customization notes
...
// Delete a product from a specific collection (Album Printing Service)
exports.deleteCollectionProduct = async (req, res) => {
  try {
    const product = await CollectionProduct.findById(req.params.productId);
