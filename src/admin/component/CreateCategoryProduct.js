...
const CreateCategoryProduct = () => {
  ...
  const [eventType, setEventType] = useState('Wedding'); // New field for event type
  const [duration, setDuration] = useState(''); // New field for duration
  const [additionalServices, setAdditionalServices] = useState(''); // New field for additional services
  ...
  const handleSubmit = async (e) => {
    ...
    formData.append('eventType', eventType); // Add event type to the form data
    formData.append('duration', duration); // Add duration to the form data
    formData.append('additionalServices', additionalServices); // Add additional services to the form data
  ...
  const resetForm = () => {
    ...
    setSize('small');
    setDescription('');
    setEventType('Wedding'); // Reset event type
    setDuration(''); // Reset duration
    setAdditionalServices(''); // Reset additional services
    setImages([]);
  ...
  const handleEdit = (product) => {
    ...
    setEventType(product.eventType); // Set event type for editing
    setDuration(product.duration); // Set duration for editing
    setAdditionalServices(product.additionalServices); // Set additional services for editing
  };
    ...
    <div className="min-h-screen bg-gray-100 flex flex-col items-center justify-center py-10">
      ...
      <div className="bg-white p-8 rounded-lg shadow-lg max-w-lg w-full mb-12">
        ...
        <form onSubmit={handleSubmit} className="mb-8 space-y-6">
          ...
          <div className="space-y-2">
            <label className="block text-gray-700 text-sm font-semibold">Event Type</label>
            <select
              value={eventType}
          ...
          <div className="space-y-2">
            <label className="block text-gray-700 text-sm font-semibold">Duration (Hours)</label>
            <input
              type="number"
              value={duration}
          ...
          <div className="space-y-2">
            <label className="block text-gray-700 text-sm font-semibold">Additional Services</label>
            <input
              type="text"
              value={additionalServices}
              onChange={(e) => setAdditionalServices(e.target.value)}
              className="w-full p-3 border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition"
              placeholder="Enter additional services (e.g., Drone, Multi-camera)"
            />
          </div>
          ...
          <div className="space-y-2">
            <label className="block text-gray-700 text-sm font-semibold flex items-center">
              ...
              {uploadProgress > 0 && <span className="ml-4 text-blue-500">{uploadProgress}%</span>}
            </label>
            <input
              type="file"
              multiple
            ...
            {products.map((product) => (
              ...
              >
                <span className="text-gray-700">
                  {product.name} - ₹{product.price} - {product.size} - {product.eventType} - {product.duration} hours
                </span>
                <div>
                  <button
